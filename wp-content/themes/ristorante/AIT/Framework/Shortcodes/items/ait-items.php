        ...
        $category = get_the_category_list( ', ', $parents = '', $latest_post['0']->ID );
          ...
          if (has_post_thumbnail( $latest_post['0']->ID )) {
			...
			if(TIMTHUMB_URL)
				...
				$result .= '<img src="' . AitImageResizer::resize(wp_get_attachment_url(get_post_thumbnail_id($latest_post['0']->ID)), array('w' => 200, 'h' => 80)) . '" alt="'.$latest_post['0']->post_title.'" />';
              $result .= '</a>';
              $result .= '</div>';
              $result .= '</div>';
              $result .= '</header>';
        ...
        foreach ($latest_posts as $key => $latest_post) {
          ...
          if (has_post_thumbnail( $latest_post->ID )) {
			...
			if(TIMTHUMB_URL)
				...
				$result .= '<img src="'. AitImageResizer::resize(wp_get_attachment_url(get_post_thumbnail_id($latest_post->ID)), array('w' => 306, 'h' => 120)) . '" alt="'.$latest_post->post_title.'" />';
