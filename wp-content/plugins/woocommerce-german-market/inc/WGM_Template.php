...
class WGM_Template {
    ...
    public static function add_woocommerce_de_templates( $template, $template_name, $template_path ){
		...
		if( file_exists( $path . $template_name ) && ! locate_template( array( WC_TEMPLATE_PATH . $template_name ) ) ) {
    ...
    public static function add_mwst_rate_to_product_item( $amount, $item, $item_id ) {
		...
		elseif( get_option('woocommerce_tax_display_cart') == 'incl' ){
			...
			$tax_string = apply_filters(
                ...
                $incl_excl . ' ' . wc_price( $item[ 'line_tax' ] ) . ' (' . round( $tax[ 'rate' ], 1 ). '%) ',
				$item[ 'line_tax' ],
				$tax[ 'rate' ],
				$incl_excl
			);
    ...
    public static function add_mwst_rate_to_product_order_item( $subtotal, $item, $order_obj ) {
		...
		$tax_string = apply_filters(
				'wgm_format_vat_output',
				wc_price( $item[ 'line_tax' ] ) . ' (' . round( $tax[ 'rate' ], 1 ) . '%) ',
				$item[ 'line_tax' ],
				$tax[ 'rate' ],
				$incl_excl
			);
	...
	*/
	public static function woocommerce_after_cart_totals() {
		...
		$html  = '<div class="second-checkout-button-container">';
		$html .= '<a href="' . esc_url( $checkout_url ) . '" class="checkout-button button alt wc-forward second-checkout-button">';
		...
		if ($price_html = $product->get_price_html() ) : ?>
			<div class="price <?php echo $classes;?>" >
				...
				if( ! empty( $price_per_unit_data ) && ( get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_show_price_per_unit' ) ) == 'on' ) ) {
					...
					if( get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_kleinunternehmerregelung' ) ) == 'on' ) {
                        do_action( 'wgm_before_kleinunternehmerreglung_notice_loop' );
						echo '<div class="wgm-kleinunternehmerregelung">';
						echo apply_filters( 'woocommerce_de_small_business_regulation_text', __( 'Umsatzsteuerbefreit nach §19 UstG', Woocommerce_German_Market::get_textdomain() ) );
						echo '</div>';
                        do_action( 'wgm_after_kleinunternehmerreglung_notice_loop' );
					}
	...
	*/
	public static function woocommerce_de_price_with_tax_hint_single() {
		...
		<div class="price <?php echo $classes; ?>">
			...
			if( get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_kleinunternehmerregelung' ) ) == 'on' ) {
                do_action( 'wgm_before_kleinunternehmerreglung_notice_single' );
				echo '<div class="wgm-kleinunternehmerregelung">';
				echo apply_filters( 'woocommerce_de_small_business_regulation_text', __( 'Umsatzsteuerbefreit nach §19 UstG', Woocommerce_German_Market::get_textdomain() ) );
				echo '</div>';
                do_action( 'wgm_after_kleinunternehmerreglung_notice_single' );
			}
    ...
    public static function add_gateway_fees(){
        ...
        foreach( WGM_Gateways::get_gateway_fees() as $gateway => $fee ){
            ...
            if( isset($_SESSION[ 'first_checkout_post_array' ][ 'payment_method' ]) &&
                ...
                $g = $avail[ $gateway ];
                WC()->cart->add_fee( __( $g->title , Woocommerce_German_Market::get_textdomain() ), $fee, true, 'standard' );
            }
        }
    }
	...
	*/
	public static function add_shipping_time_to_product_title( $item_name, $item ) {
		...
		return $item_name . ' ( ' . __( 'jeweils', Woocommerce_German_Market::get_textdomain() ) .  ' ' . wc_price( $product->price ) . $shipping_time_output . ' ) ';
	}
		...
		if ( $is_taxable ) :
			...
			<span class="woocommerce-de_price_taxrate <?php echo $classes; ?>">
				...
				foreach( $tax_rates as $rate )
					...
					if( get_option( WGM_Helper::get_wgm_option( 'woocommerce_de_kleinunternehmerregelung' ) ) == 'on' ) {
                        do_action( 'wgm_before_kleinunternehmerreglung_tax_notice' );
						echo apply_filters( 'woocommerce_de_small_business_regulation_text', __( 'Umsatzsteuerbefreit nach §19 UstG', Woocommerce_German_Market::get_textdomain() ) );
                        do_action( 'wgm_after_kleinunternehmerreglung_tax_notice' );
					} else {
						if ( $tax_print_include_enabled ) {
                            $tax_display = apply_filters( 'wgm_tax_text', sprintf( '%s %s%% %s', $include_string,
                                round( $rate[ 'rate' ], 1 ), $rate[ 'label' ] ), $product, $include_string, $rate );
							echo $tax_display;
                        } else {
	...
	*/
    ...
    public static function filter_order_item_name( $items, $order ){
        ...
        foreach( $items as &$item ){
            $search = apply_filters(
                'wgm_product_name_virtual_notice',
                sprintf( '<span class="wgm-virtual-notice">%s</span>', $keyword ),
                $item[ 'name' ], $keyword
            );
