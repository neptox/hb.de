...
<div id="order_review">
			...
			<?php if ( $woocommerce->cart->needs_shipping() && $woocommerce->cart->show_shipping() ) : ?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if( WC()->cart->get_cart_shipping_total() !== 'Free!' && get_option('woocommerce_de_kleinunternehmerregelung') != 'on' ) {
				...
				<td>
					...
					<?php
						...
						if ( $woocommerce->cart->tax_display_cart == 'incl' ) {
							...
							} elseif ( $woocommerce->cart->get_tax_totals() ) {
								$tax_string_array[] = sprintf( '%s tax', wc_price( $tax->mount ) );
							}
	...
	<div class="form-row place-order">
		...
		<input type="submit" class="button alt" name="woocommerce_checkout_update_totals" id="place_order_back" value="<?php _e( 'Zurück', Woocommerce_German_Market::get_textdomain() ) ?>" />
		<input type="submit" class="button alt wgm-place-order" name="woocommerce_checkout_place_order" id="place_order" value="<?php echo apply_filters('woocommerce_de_buy_button_text', __( 'Kaufen', Woocommerce_German_Market::get_textdomain() ) ); ?>" />
		<?php
