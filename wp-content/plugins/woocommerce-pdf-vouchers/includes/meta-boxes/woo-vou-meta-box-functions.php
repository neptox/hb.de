...
function woo_vou_add_text( $args, $echo = true ) {  
	...
	$html .= "<input type='text' class='woo-vou-meta-text regular-text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' />";
...
function woo_vou_add_select( $args, $echo = true ) {
	...
	$html .= "<select class='woo-vou-meta-select ".($field['multiple'] ? 'woo-vou-meta-multiple-select' : 'woo-vou-meta-single-select')."' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . ">";
